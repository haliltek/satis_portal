...
@section('title', 'B2B Admin Panel - Fiyat Düzenle')
@section('content')
    <!-- ============================================================== -->
    ...
    <div class="main-content">
        ...
        <div class="page-content">
                    ...
                    <div class="page-title-box d-flex align-items-center justify-content-between">
                        ...
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                ...
                                <li class="breadcrumb-item active">Fiyat Düzenle</li>
                            </ol>
                        </div>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">Fiyat Düzenle
                            <a class="btn btn-danger float-right" href="{{url('panel/fiyatayarlari')}}">Geri</a>
                        </div>
                        <div class="card-body">

                            <div data-repeater-list="group-a">
                                <form action="{{url('panel/fiyatduzenlepost')}}/{{$id}}" method="post">
                                    @csrf
                                    <div data-repeater-item class="row">
                                        <div class="form-group col-lg-3">
                                            <label for="name">Fiyat Adı</label>
                                            <input type="text" id="name" value="{{$name}}" name="name" class="form-control" placeholder="" />
                                        </div>
                                        <div class="form-group col-lg-3">
                                            <label for="name">Para Birimi</label>
                                            <select name="birim" name="birim" class="form-control">
                                                @foreach($pbirim as $birim)
                                                    <option value="{{$birim->id}}" @if($birimi==$birim->id)selected @endif>{{$birim->isim}}</option>
                                                @endforeach
                                            </select>
                                        </div>
