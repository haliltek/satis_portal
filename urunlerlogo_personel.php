...
if (isset($_POST['action'])) {
    ...
    if ($action === 'getDetails') {
        ...
        exit;
    } elseif ($action === 'request_price_update') {
        $stok = trim($_POST['stok_kodu'] ?? '');
        $urun = trim($_POST['urun_adi'] ?? '');
        $note = trim($_POST['note'] ?? '');
        $user = $_SESSION['yonetici_adsoyad'] ?? 'Bilinmeyen Kullanıcı';
        $userId = $_SESSION['yonetici_id'] ?? 0;

        if (empty($stok) || empty($note)) {
            echo json_encode(['status' => 'error', 'message' => 'Lütfen notunuzu giriniz.']);
            exit;
        }

        // 1. Veritabanına Kaydet (fiyat_talepleri)
        try {
            // Tablo sütunları: talep_id, stokkodu, stokadi, talep_eden_adi, talep_tarihi, talep_notu, durum, onerilen_fiyat, talep_eden_id
            $stmt = $db->prepare("INSERT INTO fiyat_talepleri (stokkodu, stokadi, talep_eden_adi, talep_eden_id, talep_tarihi, talep_notu, durum, onerilen_fiyat) VALUES (?, ?, ?, ?, NOW(), ?, 'Beklemede', 0)");
            if ($stmt) {
                // bind_param: s (string), s(string), s(string), i(int), s(string)
                $stmt->bind_param("sssis", $stok, $urun, $user, $userId, $note);
                $stmt->execute();
                $stmt->close();
            }
        } catch (Exception $e) {
            $logger->log("DB Insert Error: " . $e->getMessage());
        }
        ...
        $subject = "Fiyat Güncelleme Talebi: $stok";
        $body = "<h3>Fiyat Güncelleme Talebi</h3><p><strong>Talep Eden:</strong> $user</p><p><strong>Stok Kodu:</strong> $stok</p><p><strong>Ürün:</strong> $urun</p><p><strong>Talep Notu:</strong><br>" . nl2br(htmlspecialchars($note)) . "</p><hr><small>B2B Panel</small>";
        
        try {
            foreach ($recipients as $email => $name) {
                $mailService->sendMail($email, $name, $subject, $body);
            }
            echo json_encode(['status' => 'success', 'message' => 'Talebiniz kaydedildi ve iletildi.']);
        } catch (Exception $e) {
            $logger->log("Mail Error: " . $e->getMessage());
            // Mail gitmese bile DB kaydı başarılıysa success dönelim
            echo json_encode(['status' => 'success', 'message' => 'Talebiniz sisteme kaydedildi.']);
        }
...
<body data-layout="horizontal" data-topbar="colored">
                            ...
                            <div class="card shadow-sm">
                                <div class="card-body">
                                    ...
                                    <div class="table-responsive">
                                        <table id="example_personel" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%;">
    ...
    <div class="modal fade" id="yenikategoriModal" tabindex="-1" aria-labelledby="yenikategoriModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <form id="newMaterialForm" method="post" action="urunlerlogo_personel.php?ajax=1" novalidate>
    ...
    <div class="modal fade" id="requestPriceModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form id="requestPriceForm">
                <input type="hidden" name="action" value="request_price_update">
                <input type="hidden" name="stok_kodu" id="req_stok_kodu">
                <input type="hidden" name="urun_adi" id="req_urun_adi">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title"><i class="bi bi-send me-2"></i>Fiyat Güncelleme Talebi</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Kapat"></button>
                    </div>
                    <div class="modal-body">
                        <p><strong>Stok Kodu:</strong> <span id="req_display_stok"></span></p>
                        <p><strong>Ürün:</strong> <span id="req_display_urun"></span></p>
                        <div class="mb-3">
                            <label class="form-label fw-bold">Talep Notunuz / Önerilen Fiyat <span class="text-danger">*</span></label>
                            <textarea class="form-control" name="note" rows="4" required placeholder="Lütfen talep nedenini ve önerilen fiyatı yazınız..."></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
            ...
            <form id="requestPriceForm">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-bold">Talep Notunuz / Önerilen Fiyat <span class="text-danger">*</span></label>
                            <textarea class="form-control" name="note" rows="4" required placeholder="Lütfen talep detaylarını yazınız..."></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">İptal</button>
    ...
    <div class="modal fade" id="bulkModal" tabindex="-1" aria-labelledby="bulkModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-xxl modal-dialog-scrollable" style="max-width:95vw;">
            <form id="bulkUploadForm" method="post" enctype="multipart/form-data" action="urunlerlogo_personel.php">
                <div class="modal-content">
    ...
    <script>
        $(document).ready(function() {
            ...
            if (searchTerm) {
                ...
                var table = $('#example_personel').DataTable();
            ...
            // Personel Tablosu Init
            $('#example_personel').DataTable({
                "processing": true,
                "serverSide": true,
                "ajax": "urunlerlogo_datatable_personel.php",
                "responsive": true,
                "language": {
