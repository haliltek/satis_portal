...
if (isset($_POST['add_new_material_card'])) {
    ...
    if ($isAjax) {
        ...
        header('Location: urun_fiyat_onerisi.php');
        exit;
    }
}
...
<body data-layout="horizontal" data-topbar="colored">
                    ...
                    <div class="row mb-4">
                        <div class="col-12">
                            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                                ...
                                <!-- Toplu işlem butonu kaldırıldı -->
                                <!-- Logo senkronizasyonu kaldırıldı -->
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card shadow-sm border-info">
                                <div class="card-body">
                                    <h5 class="text-info"><i class="bi bi-info-circle me-2"></i> Fiyat Öneri Sistemi</h5>
                                    <p class="mb-0">Aşağıdaki tabloda ürünlerin güncel fiyatlarını görebilirsiniz. Fiyatlarda bir yanlışlık olduğunu düşünüyorsanız veya güncelleme gerekiyorsa <strong>"Öneri Ver"</strong> butonuna tıklayarak talebinizi notunuzla birlikte iletebilirsiniz.</p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <!-- Fiyat Önerisi Modal -->
    <div class="modal fade" id="suggestPriceModal" tabindex="-1" aria-labelledby="suggestPriceModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form id="suggestPriceForm">
                <input type="hidden" name="urun_id" id="suggest_urun_id">
                <input type="hidden" name="stok_kodu" id="suggest_stok_kodu">
                <input type="hidden" name="mevcut_fiyat_yurtici" id="suggest_mevcut_fiyat_yurtici">
                <input type="hidden" name="mevcut_fiyat_export" id="suggest_mevcut_fiyat_export">
                
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="suggestPriceModalLabel">Fiyat Güncelleme Önerisi Ver</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Kapat"></button>
                    </div>
                    <div class="modal-body">
                        <div class="mb-3">
                            <label class="form-label fw-bold">Ürün:</label>
                            <div id="suggest_urun_adi_display" class="alert alert-light p-2 border"></div>
                        </div>
                        <div class="row mb-3">
                            <div class="col-6">
                                <label class="form-label">Mevcut Yurtiçi</label>
                                <input type="text" class="form-control bg-light" id="suggest_mevcut_fiyat_yurtici_display" readonly>
                            </div>
                            <div class="col-6">
                                <label class="form-label">Mevcut Export</label>
                                <input type="text" class="form-control bg-light" id="suggest_mevcut_fiyat_export_display" readonly>
                            </div>
                        </div>
                        <hr>
                        <!-- Öneri fiyat alanları kaldırıldı, kullanıcı sadece not girecek -->
                        <div class="mb-3">
                            <label for="oneri_not" class="form-label">Notunuz / Gerekçeniz</label>
                            <textarea class="form-control" name="oneri_not" id="oneri_not" rows="3" placeholder="Neden bu fiyatı öneriyorsunuz?"></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
    ...
    <script>
        $(document).ready(function() {
            ...
            var ajaxUrl = pagePath.replace(/urun_fiyat_onerisi\.php$/, 'urun_fiyat_onerisi_datatable.php');
            if (ajaxUrl === pagePath) {
                ajaxUrl = 'urun_fiyat_onerisi_datatable.php';
            }
    ...
    <script>
            // Fiyat Önerisi Butonuna Tıklanması
            $(document).on('click', '.suggest-price-btn', function() {
                var btn = $(this);
                var urun_id = btn.data('id');
                var stok_kodu = btn.data('stokkodu');
                var urun_adi = btn.data('urunadi');
                var domestic = btn.data('domestic');
                var export_p = btn.data('export');

                $('#suggest_urun_id').val(urun_id);
                $('#suggest_stok_kodu').val(stok_kodu);
                $('#suggest_mevcut_fiyat_yurtici').val(domestic);
                $('#suggest_mevcut_fiyat_export').val(export_p);
                
                $('#suggest_urun_adi_display').text(stok_kodu + ' - ' + urun_adi);
                $('#suggest_mevcut_fiyat_yurtici_display').val(domestic);
                $('#suggest_mevcut_fiyat_export_display').val(export_p);
                
                // Öneri alanlarını sıfırla
                $('#oneri_not').val('');

                $('#suggestPriceModal').modal('show');
            });

            // Fiyat Önerisi Formu Gönderimi
            $('#suggestPriceForm').on('submit', function(e) {
                e.preventDefault();
                var formData = $(this).serialize();
                $.ajax({
                    url: 'api/fiyat_onerisi_kaydet.php',
                    type: 'POST',
                    data: formData,
                    dataType: 'json',
                    success: function(response) {
                        if (response.success) {
                            alert('Öneriniz başarıyla kaydedildi.');
                            $('#suggestPriceModal').modal('hide');
                        } else {
