...
<body data-layout="horizontal" data-topbar="colored">
                        ...
                        <div class="col-lg-12">
                            ...
                            if (isset($_POST['duzenleme'])) {
                                ...
                                if ($duzenleme) {
                                    ...
                                    echo '<meta http-equiv="refresh" content="2; url=reddedilenuyeler.php"> ';
                                } else {
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <h4 class="card-title mb-4"><b>Red Edilmiş</b> Şirket Personel Başvuruları</h4>
                                    <div class="table-responsive">
                                        <table id="datatable" class="table table-bordered table-responsive nsowrap"
                                            ...
                                            <tbody>
                                                <?php
                                                $i = 1;
                                                $personel_sorgulama = mysqli_query($db, "SELECT * FROM  personel where p_durum='Red'");
                                                while ($personel = mysqli_fetch_array($personel_sorgulama)) {
                                                ?>
                                                    <tr>
                                                        <td><?php echo $i; ?></td>
                                                        <td><?php echo $personel["p_adi"]; ?>
                                                            <?php echo $personel["p_soyadi"]; ?></td>
                                                        <td>
                                                            <?php
                                                        ...
                                                        <td>
                                                            <button type="button"
                                                                ...
                                                                data-bs-target=".duzenle<?php echo $personel["personel_id"]; ?>">
                                                                Düzenle </button>
                                                            <?php $dur = $personel["p_durum"];
                                                            if ($dur == 'Onaylı') {
                                                                echo '<a href="bekleyenhesaponayinikaldir.php?id=' . $personel["personel_id"] . '&durum=Beklemede" class="btn btn-warning waves-effect waves-light btn-sm">  Hesabı Beklemeye Al! </a> 
                                                    <a href="bekleyenhesaponayinikaldir.php?id=' . $personel["personel_id"] . '&durum=Red" class="btn btn-warning waves-effect waves-light btn-sm">  Hesabı Reddet! </a>';
                                                            } else
                                                                if ($dur == 'Beklemede') {
                                                                echo '<a href="bekleyenhesaponayinikaldir.php?id=' . $personel["personel_id"] . '&durum=Onaylı" class="btn btn-success waves-effect waves-light btn-sm">  Hesabı Onayla! </a>
                                                    <a href="bekleyenhesaponayinikaldir.php?id=' . $personel["personel_id"] . '&durum=Red" class="btn btn-warning waves-effect waves-light btn-sm">  Hesabı Reddet! </a>';
                                                            } else
                                                                    if ($dur == 'Red') {
                                                                echo '<a href="bekleyenhesaponayinikaldir.php?id=' . $personel["personel_id"] . '&durum=Beklemede" class="btn btn-warning waves-effect waves-light btn-sm">  Hesabı Beklemeye Al! </a>';
                                                            } ?>
                                                            <a href="bekleyensirketpersonelsil.php?id=<?php echo $personel["personel_id"]; ?>"
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-12">
                            <b>GENEL TANIM</b>
                            <p>Şirket alanı site üzerinden kayıt gerçekleştiren şirketlere ait verilerdir. Bu alanda
                                onaylı, onaysız veya bekleyen tüm şirket kayıtlarına erişim sağlayabilirsiniz. </p>
                        </div>
                    </div>
                </div>
    ...
    <?php
    $sirket_sorgulama = mysqli_query($db, "SELECT * FROM  personel where p_durum='Red'");
    while ($sirket = mysqli_fetch_array($sirket_sorgulama)) {
    ?>
        <div class="modal fade duzenle<?php echo $sirket["personel_id"]; ?>" tabindex="-1" role="dialog"
            aria-labelledby="myLargeModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="myLargeModalLabel"><b><?php echo $sirket["p_adi"]; ?>
                                <?php echo $sirket["p_soyadi"]; ?></b> Düzenleyin!</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                        </button>
                    </div>
                    <form method="post" action="reddedilenuyeler.php" class="needs-validation" novalidate>
                        <div class="modal-body">
                            <div class="p-2 mt-2">
                                <form action="kayitol.php" method="post">
                                    <div class="row">
                                        <div class="mb-3 col-md-6">
                                            ...
                                            <input required type="text" class="form-control" id="username" name="adi"
                                                value="<?php echo $sirket["p_adi"]; ?>" placeholder="ÖRN: Erkan">
                                        </div>
                                        <div class="mb-3 col-md-6">
                                            <label class="form-label" for="username"> Soyadı</label>
                                            <input required type="text" class="form-control" id="username"
                                                value="<?php echo $sirket["p_soyadi"]; ?>" name="soyadi"
                                                placeholder="ÖRN: AK">
                                        </div>
                                    </div>
                                    <div class="mb-3">
                                        ...
                                        <input required type="email" class="form-control" id="email"
                                            value="<?php echo $sirket["p_eposta"]; ?>" name="eposta"
                                            placeholder="ÖRN: info@gemas.com">
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label" for="username">Cep Telefon Numarası</label>
                                        <input required type="number" maxlength="11" value="<?php echo $sirket["p_cep"]; ?>"
                                            class="form-control" id="username" name="cep" placeholder="ÖRN: 055555555">
                                    </div>
